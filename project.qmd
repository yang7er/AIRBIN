...
grid_search <- function(X, y, model, hyperparameter_grid, k){
  ...
  for(i in 1:nrow(hyperparameter_grid)){
    ...
    results[[i%%(10 + 1)]] <- as.list(rep_len(NA, k))
    # loop over folds
    for(j in 1:k){
      print(paste0("~~~ fold: ", j, " of ", k, " ~~~"))
      results[[i%%(10 + 1)]][[j]] <- model(hyperparameter_grid[i,] %>% as.list()) %>% 
      fit(
...
```{r}
final_predict <- function(mod, analysis_data, start_date = "2023-10-01", end_date = "2024-09-30"){
  ...
  for(n_beds in 0:1){
    ...
    X_lagged_predict[1,,] <- analysis_data %>% 
      filter(beds == n_beds, date == (ymd(start_date) - 1)) %>%
      select(`lag(mean_price, 1)`:`lag(mean_price, 28)`) %>%
      as.matrix()
    ...
    y_predict <- raster_predict %>% 
      filter(beds == n_beds) %>% 
      select(date, beds, mean_price)

    for(i in 1:(nrow(X_predict[[1]]) - 1)){
      #
      y_predict$mean_price[[i]] <- mod[[(n_beds + 1)]] %>% predict(list(X_predict[[1]][i,,,drop=F], X_predict[[2]][i,,drop=F]), verbose = 0)
      #
      X_predict[[1]][i+1,,] <- c(y_predict$mean_price[[i]], X_predict[[1]][i,1:27,])
    }
    out[[n_beds + 1]] <- y_predict
  }
...
hyperparameters_grid <- list(
    ...
    init_dropout = c(0.1, 0.3),                # Initial dropout rate
...
run_4 <- final_fit(list(X_lagged_4, X_external), y_4, hyperparameter_grid[1,] %>% as.list(), lstmnet)
run_6 <- final_fit(list(X_lagged_6, X_external), y_6, hyperparameter_grid[1,] %>% as.list(), lstmnet)

plot(run[[2]])
```

```{r}
raster_predict <- final_predict(list(run_4[[1]], run_6[[1]]), analysis_data)

(visualize_data %>%
    ggplot() +
      geom_line(data = analysis_data, aes(x = date, y = descale_prices(mean_price, full_data), linetype = "dashed", group = beds)) +
      geom_line(data = raster_predict, aes(x = date, y = descale_prices(mean_price, full_data), color = beds, group = beds)))
